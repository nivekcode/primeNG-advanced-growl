...
|messageSpots (default: 0 (unlimited))| This property gives you the possibility to limit the number of messages displayed on the screen. This features is very useful if you also set the life property. In case you limit the message spots and set a life time property all the messages that can not appear on screen as there are not enough spots will be cached and displayed after the first message dissapears. Notice that a dynamic change of this property during runtime removes all currently displayed messages from screen|
